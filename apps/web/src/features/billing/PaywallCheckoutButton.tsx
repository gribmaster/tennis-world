...
// (Feature 67; plans reworked to monthly/quarterly/yearly). Replaces the Phase-1 inert
// `<button disabled>` in PaywallModal.
//
// On click it starts a hosted Stripe Checkout for the given recurring `plan` via
...
// handles that). A failure (unconfigured Stripe → 500, unoffered plan → 400, mock mode
// → not-available, or a network blip) shows a calm inline error under the button and
// does NOT navigate.
//
...
export interface PaywallCheckoutButtonProps {
  /** Which recurring plan this button starts checkout for. */
  plan: BillingPlanKey;
