...
// CollectionCourtSaveHeart — the circular save/unsave heart overlaid on a Collection
// Detail court card (Task 47), restyled to the same Featured-courts card language:
...
// Collection Detail) here would tie two unrelated screens together for a component with
// no shared parent. The BEHAVIOUR below is deliberately reproduced rule for rule from
// `HomeCourtSaveHeart`, and the CLAUDE.md §4 rule-2 triad:
...
//     `redirectTo` back to this court, matching `HomeCourtSaveHeart` exactly,
...
export interface CollectionCourtSaveHeartProps {
  courtId: string;
  courtSlug: string;
  /**
...
export function CollectionCourtSaveHeart({
  courtId,
  courtSlug,
  courtLabel,
  initialSaved,
  signedIn = true,
}: CollectionCourtSaveHeartProps) {
