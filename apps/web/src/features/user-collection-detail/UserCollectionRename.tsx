...
export function UserCollectionRename({ collectionId, name }: UserCollectionRenameProps) {
  ...
  // In-flight rename write. Disables Save + shows an inline spinner; always reset (even on
  // failure) so a failed rename never leaves the button stuck disabled.
  const [pending, setPending] = useState(false);
  // Set on a failed rename; cleared on the next edit/attempt. Kept minimal (no toast system
  // in this app) but still surfaced — errors are never silently swallowed.
  const [error, setError] = useState<string | null>(null);

  const trimmed = draft.trim();
  // Save is disabled while the trimmed name is empty, unchanged, or a save is in flight.
  ...
  async function save() {
    ...
    try {
      ...
      // Stay in edit mode with the draft intact so the user can retry; never leave the
      // button stuck disabled (the `finally` below always clears `pending`).
      setError('Couldn’t rename this collection. Please try again.');
    } finally {
      setPending(false);
    }
  }
  ...
  if (editing) {
    return (
      <div className="flex flex-1 flex-col gap-1.5">
        <div className="flex items-center gap-2.5" style={{ minWidth: 240 }}>
          ...
          >
            {pending ? <InlineSpinner label="Saving…" /> : null}
            {pending ? 'Saving…' : 'Save'}
          </button>
          <button
            type="button"
        ...
        {error ? (
          <p role="alert" className="body-s text-clay">
            {error}
          </p>
        ) : null}
      </div>
